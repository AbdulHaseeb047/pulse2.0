...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 left-0 right-0 z-50 w-full bg-emerald-950/90 font-headline font-bold tracking-tight text-emerald-500 shadow-xl shadow-emerald-900/10 backdrop-blur-2xl">
      ...
      <Link to="/" className="text-xl md:text-2xl font-black text-white italic leading-none tracking-[0.03em]">
        Emerald Pulse
      </Link>
      ...
      <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
        {navItems.map((item) => (
          <Link
            key={item.label}
            to={item.to}
            className={`nav-link text-xs uppercase tracking-[0.16em] font-bold ${isActive(item.label) ? "active text-white" : "text-white/90 hover:text-white"}`}
          >
            {item.label}
          </Link>
        ))}
      ...
      <div className="flex shrink-0 items-center space-x-2.5 md:space-x-4">
        ...
        <Link to="/connect" className="rounded-lg bg-primary px-4 py-2 text-[11px] font-black uppercase tracking-[0.14em] text-on-primary shadow-md shadow-primary/20 transition-all hover:brightness-110 active:scale-95 md:px-5 md:py-2.5 md:text-xs">
          Book Now
        </Link>
      </div>
      </div>
